...
func (n *Node) connect(opts ...grpc.DialOption) error {
  ...
  {{- /* Note that if a connect fails below, it returns, leaving the n.conn connected. */ -}}
  {{- /* It is not clear what we should do in this case; should n.conn be closed when returning an error? */ -}}
  {{- /* And will we ever have more than one clientServName? */ -}}

{{$serviceName := ""}}
{{range $elm := .Services}}
{{if .Streaming}}
{{if ne .ServName $serviceName}}
    client{{.ServName}} := New{{.ServName}}Client(n.conn)
{{$serviceName := .ServName}}
{{- end -}}
  	n.{{.UnexportedMethodName}}Client, err = client{{.ServName}}.{{.MethodName}}(context.Background())
  	if err != nil {
