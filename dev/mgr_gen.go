...
// Template file to edit is: mgr.tmpl
...
func NewManager(nodeAddrs []string, opts ...ManagerOption) (*Manager, error) {
	...
	m := &Manager{
		...
		writeOrderedRecv: make(map[uint32]chan *internalWriteResponse),
	}
...
func (m *Manager) createNode(addr string) (*Node, error) {
	...
	node := &Node{
		...
		writeOrderedSend: make(chan *State),
		writeOrderedRecv: m.writeOrderedRecv,
		writeOrderedLock: &m.writeOrderedLock,
	}
...
func (m *Manager) nextWriteOrderedID() uint32 {
	return atomic.AddUint32(&m.writeOrderedID, 1)
}
