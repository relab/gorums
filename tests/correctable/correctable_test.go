...
func run(t *testing.T, n int, div int, corr func(*Configuration) *gorums.Correctable) {
...
func TestCorrectable(t *testing.T) {
	run(t, 4, 1, func(c *Configuration) *gorums.Correctable {
		corr := c.Correctable(context.Background(), &CorrectableRequest{})
		return corr.Correctable
	})
}

func TestCorrectableStream(t *testing.T) {
	run(t, 4, 4, func(c *Configuration) *gorums.Correctable {
		corr := c.CorrectableStream(context.Background(), &CorrectableRequest{})
		return corr.Correctable
	})
}
