...
func run(t *testing.T, n int, div int, corr func(context.Context, *Configuration[uint32]) *gorums.Correctable) {
	...
	cfg, err := mgr.NewConfiguration(qspec{div, n}, gorums.WithNodeList[uint32](addrs))
	if err != nil {
		t.Fatal(err)
	}
...
func TestCorrectable(t *testing.T) {
	run(t, 4, 1, func(ctx context.Context, c *Configuration[uint32]) *gorums.Correctable {
...
func TestCorrectableStream(t *testing.T) {
	run(t, 4, 4, func(ctx context.Context, c *Configuration[uint32]) *gorums.Correctable {
