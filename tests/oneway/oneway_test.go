...
func setup(t testing.TB, cfgSize int) (cfg *oneway.Configuration[uint32], srvs []*onewaySrv, teardown func()) {
	t.Helper()
...
func TestOnewayCalls(t *testing.T) {
	...
	f := func(c *oneway.Configuration[uint32]) func(context.Context, *oneway.Request, ...gorums.CallOption) {
		if c.Size() == 1 {
