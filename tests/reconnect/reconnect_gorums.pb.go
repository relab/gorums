...
func (n *Node) Test(ctx context.Context, in *Echo, opts ...grpc.CallOption) (resp *Echo, err error) {
...
// QuorumSpec is the interface of quorum functions for Reconnect.
type QuorumSpec interface {
}

// Reconnect is the server-side API for the Reconnect Service
type Reconnect interface {
	Test(context.Context, *Echo, func(*Echo, error))
}

func (s *GorumsServer) RegisterReconnectServer(srv Reconnect) {
	s.srv.handlers[testMethodID] = func(ctx context.Context, in *gorumsMessage, finished chan<- *gorumsMessage) {
		req := in.message.(*Echo)
		once := new(sync.Once)
...
var orderingMethods = map[int32]methodInfo{
	...
	0: {requestType: new(Echo).ProtoReflect(), responseType: new(Echo).ProtoReflect()},
}
