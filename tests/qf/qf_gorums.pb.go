...
type nodeServices struct {
	QuorumFunctionClient
}

func (n *Node) connectStream(ctx context.Context) (err error) {

	n.QuorumFunctionClient = NewQuorumFunctionClient(n.conn)
...
func (c *Configuration) UseReq(ctx context.Context, in *Request, opts ...grpc.CallOption) (resp *Response, err error) {
	var ti traceInfo
	if c.mgr.opts.trace {
		ti.Trace = trace.New("gorums."+c.tstring()+".Sent", "UseReq")
		defer ti.Finish()
	...
	for {
		...
		case r := <-replyChan:
			...
			if resp, quorum = c.qspec.UseReqQF(in, replyValues); quorum {
				return resp, nil
			}
		case <-ctx.Done():
...
func (n *Node) UseReq(ctx context.Context, in *Request, replyChan chan<- internalResponse) {
	reply := new(Response)
	start := time.Now()
	err := n.conn.Invoke(ctx, "/gorums.tests.qf.QuorumFunction/UseReq", in, reply)
...
func (c *Configuration) IgnoreReq(ctx context.Context, in *Request, opts ...grpc.CallOption) (resp *Response, err error) {
	var ti traceInfo
	if c.mgr.opts.trace {
		ti.Trace = trace.New("gorums."+c.tstring()+".Sent", "IgnoreReq")
		defer ti.Finish()
	...
	replyChan := make(chan internalResponse, expected)
	for _, n := range c.nodes {
		go n.IgnoreReq(ctx, in, replyChan)
	}
	...
	for {
		...
		case r := <-replyChan:
			...
			replyValues = append(replyValues, r.reply)
			if resp, quorum = c.qspec.IgnoreReqQF(in, replyValues); quorum {
				return resp, nil
			}
		case <-ctx.Done():
			return resp, QuorumCallError{ctx.Err().Error(), len(replyValues), errs}
...
func (n *Node) IgnoreReq(ctx context.Context, in *Request, replyChan chan<- internalResponse) {
	reply := new(Response)
	start := time.Now()
	err := n.conn.Invoke(ctx, "/gorums.tests.qf.QuorumFunction/IgnoreReq", in, reply)
	s, ok := status.FromError(err)
	if ok && (s.Code() == codes.OK || s.Code() == codes.Canceled) {
		n.setLatency(time.Since(start))
	} else {
