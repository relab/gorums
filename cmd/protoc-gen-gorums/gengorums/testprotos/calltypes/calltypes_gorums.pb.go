...
type nodeServices struct {
	CalltypesClient
	multicastClient Calltypes_MulticastClient
}

func (n *Node) connectStream(ctx context.Context) (err error) {

	n.CalltypesClient = NewCalltypesClient(n.conn)

	n.multicastClient, err = n.CalltypesClient.Multicast(ctx)
	if err != nil {
...
func (n *Node) QuorumCallFuture(ctx context.Context, in *Request, replyChan chan<- internalResponse) {
	reply := new(Response)
	start := time.Now()
	err := n.conn.Invoke(ctx, "/dev.Calltypes/QuorumCallFuture", in, reply)
...
// QuorumSpec is the interface of quorum functions for Calltypes.
...
func (n *Node) QuorumCall(ctx context.Context, in *Request, replyChan chan<- internalResponse) {
	reply := new(Response)
	start := time.Now()
	err := n.conn.Invoke(ctx, "/dev.Calltypes/QuorumCall", in, reply)
	s, ok := status.FromError(err)
	if ok && (s.Code() == codes.OK || s.Code() == codes.Canceled) {
		n.setLatency(time.Since(start))
	} else {
