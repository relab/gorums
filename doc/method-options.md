...
The client-side API is significantly different from gRPC's unary API in that it returns a `*Responses[T]` object instead of a single value.
The `Responses` object is used to collect responses from a configuration of nodes.
Users may iterate over the responses using a range loop, or use terminal methods to retrieve results directly.

There is no generated client interface or service methods.
Users should invoke Gorums' call type through generated functions.

## Async and Correctable Variants

Quorum calls support asynchronous and correctable variants through *terminal methods* on the `Responses` object.
