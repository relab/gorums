...
func (c *Configuration) AsyncQuorumCall(ctx context.Context, in *Request, opts ...grpc.CallOption) *FutureResponse {
	fut := &FutureResponse{
	...
	go func() {
		defer close(fut.c)
		c.asyncQuorumCall(ctx, in, fut, opts...)
	}()
	return fut
}

func (c *Configuration) asyncQuorumCall(ctx context.Context, in *Request, resp *FutureResponse, opts ...grpc.CallOption) {
	var ti traceInfo
	if c.mgr.opts.trace {
		ti.Trace = trace.New("gorums."+c.String()+".Sent", "AsyncQuorumCall")
		defer ti.Finish()
	...
	for _, n := range c.nodes {
		go n.AsyncQuorumCall(ctx, in, replyChan)
	}
	...
	for {
		...
		case r := <-replyChan:
			...
			if reply, quorum = c.qspec.AsyncQuorumCallQF(in, replies); quorum {
...
func (n *Node) AsyncQuorumCall(ctx context.Context, in *Request, replyChan chan<- internalResponse) {
	reply := new(Response)
	start := time.Now()
	err := n.conn.Invoke(ctx, "/gorums.testprotos.calltypes.async.AsyncQuorumCall/AsyncQuorumCall", in, reply)
...
type QuorumSpec interface {
	...
	AsyncQuorumCallQF(in *Request, replies map[uint32]*Response) (*Response, bool)
}
