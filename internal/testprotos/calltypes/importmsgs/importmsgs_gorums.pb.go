...
func (n *Node) connectStream(ctx context.Context) (err error) {
	...
	n.QuorumCallClient = NewQuorumCallClient(n.conn)
...
// QuorumSpec is the interface of quorum functions for QuorumCall.
type QuorumSpec interface {
	...
	// you should implement your quorum function with '_ *msgs.Request'.
	QuorumCallQF(in *msgs.Request, replies map[uint32]*msgs.Response) (*msgs.Response, bool)
}

// QuorumCall is a quorum call invoked on all nodes in configuration c,
// with the same argument in, and returns a combined result.
func (c *Configuration) QuorumCall(ctx context.Context, in *msgs.Request, opts ...grpc.CallOption) (resp *msgs.Response, err error) {
	var ti traceInfo
	if c.mgr.opts.trace {
	...
	var (
		errs    []GRPCError
		quorum  bool
		replies = make(map[uint32]*msgs.Response)
	)
...
func (n *Node) QuorumCall(ctx context.Context, in *msgs.Request, replyChan chan<- internalResponse) {
	reply := new(msgs.Response)
	start := time.Now()
	err := n.conn.Invoke(ctx, "/gorums.testprotos.calltypes.quorumcall.QuorumCall/QuorumCall", in, reply)
...
// QuorumCall is the server-side API for the QuorumCall Service
type QuorumCall interface {
}

func (s *GorumsServer) RegisterQuorumCallServer(srv QuorumCall) {
}
