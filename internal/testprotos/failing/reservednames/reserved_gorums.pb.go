...
type ReservedClient interface {
	QuorumCall(ctx context.Context, in *ReservedConfiguration) gorums.Responses[*ReservedNode]
}
...
type ReservedNodeClient interface {
	ConfTest(ctx context.Context, in *ReservedConfiguration) (resp *ReservedNode, err error)
}
...
func (c *ReservedConfiguration) QuorumCall(ctx context.Context, in *ReservedConfiguration) gorums.Responses[*ReservedNode] {
	...
	return gorums.QuorumCall[*ReservedNode](ctx, c.RawConfiguration, cd)
}

// ConfTest is a quorum call invoked on each node in configuration c,

// with the same argument in, and returns the responses as an iterator.
func (n *ReservedNode) ConfTest(ctx context.Context, in *ReservedConfiguration) (resp *ReservedNode, err error) {
	cd := gorums.CallData{
		Message: in,
	...
	return res.(*ReservedNode), err
}

// ReservedServer is the server-side API for the Reserved Service
type ReservedServer interface {
	ConfTest(ctx gorums.ServerCtx, request *ReservedConfiguration) (response *ReservedNode, err error)
	QuorumCall(ctx gorums.ServerCtx, request *ReservedConfiguration) (response *ReservedNode, err error)
}
...
func RegisterReservedServer(srv *gorums.Server, impl ReservedServer) {
	srv.RegisterHandler("gorums.testprotos.failing.reservednames.Reserved.ConfTest", func(ctx gorums.ServerCtx, in *gorums.Message, finished chan<- *gorums.Message) {
		req := in.Message.(*ReservedConfiguration)
		defer ctx.Release()
