...
func TestQuorumCallPartialFailures(t *testing.T) {
	...
	quorumcall := func(name string, aggregateFunc func(respType) (*pb.StringValue, error)) callInfo {
		return callInfo{
			...
			callFunc: func(ctx *gorums.ConfigContext, req *pb.StringValue) error {
				_, err := aggregateFunc(gorumsimpl.QuorumCall[*pb.StringValue, *pb.StringValue](ctx, req, mock.TestMethod))
				return err
			},
		}
	}
	...
	multicast := func(name string) callInfo {
		return callInfo{
			name: "Multicast/" + name,
			callFunc: func(ctx *gorums.ConfigContext, req *pb.StringValue) error {
				return gorumsimpl.Multicast(ctx, req, mock.TestMethod).Send()
			},
		}
	}
...
func TestQuorumCallAsyncSynctest(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		future1 := gorumsimpl.QuorumCall[*pb.StringValue, *pb.StringValue](
			cfgCtx,
			pb.String("async1"),
			mock.TestMethod,
		).AsyncMajority()

		future2 := gorumsimpl.QuorumCall[*pb.StringValue, *pb.StringValue](
			cfgCtx,
			pb.String("async2"),
			mock.TestMethod,
		).AsyncMajority()
